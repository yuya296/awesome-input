...
- ストア提出向けのアイコンを `src/assets/` に含めています。
- 公開時は、ビルド後の `out/` ディレクトリの中身をルートにした ZIP をアップロードしてください。
- スクリーンショットは ChatGPT / Gemini の両方で、改行と送信挙動が分かるものを用意してください。
...
- Awesome Input は、ユーザーのプロンプトやチャット内容を収集・保存・送信しません。
- 解析、広告、サードパーティトラッキングは行いません。
- 公開用のプライバシー文言は [docs/privacy.ja.md](/Users/yuya/dev/chatgpt-keyflow-extension/docs/privacy.ja.md) を参照してください。
